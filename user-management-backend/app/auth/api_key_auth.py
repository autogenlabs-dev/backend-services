...
class ApiKeyService:
    ...
    def validate_api_key(
        ...
        Enhanced to handle sub-user API keys
        """
        try:
            ...
            if user.is_sub_user and user.parent_user_id:
                # Get parent user for billing/subscription context
                parent_user = db.query(User).filter(User.id == user.parent_user_id).first()
                if not parent_user or not parent_user.is_active:
                    return None, None
                
                # Validate sub-user permissions
                permissions = user.sub_user_permissions or {}
                if not permissions.get("api_access", True):
                    return None, None
                
                # Check sub-user token limits
                limits = user.sub_user_limits or {}
                monthly_tokens = limits.get("monthly_tokens", user.monthly_limit)
                
                if user.tokens_used >= monthly_tokens:
                    return None, None
                
                # Add sub-user context to the key metadata
                db_key.metadata = db_key.metadata or {}
                db_key.metadata.update({
                    "is_sub_user_key": True,
                    "parent_user_id": str(user.parent_user_id),
