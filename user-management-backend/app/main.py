...
# CORS Configuration
# In production (behind Cloudflare), Cloudflare handles CORS at the edge.
# Adding CORS middleware here would cause duplicate headers which browsers reject.
# Only add CORS middleware in development/debug mode.
import os

# Check if we're in production (behind Cloudflare)
is_production = os.getenv("ENVIRONMENT") == "production" or not settings.debug
disable_cors_middleware = os.getenv("DISABLE_CORS_MIDDLEWARE", "false").lower() == "true"

# Start with origins from config
allowed_origins = list(settings.backend_cors_origins)

# Add both www and non-www versions of production domains
production_domains = [
    "https://codemurf.com",
...
print(f"   - CORS Middleware Disabled: {disable_cors_middleware}")

# Only add CORS middleware if NOT disabled (for Cloudflare production)
# In production behind Cloudflare, set DISABLE_CORS_MIDDLEWARE=true
if not disable_cors_middleware:
    app.add_middleware(
        CORSMiddleware,
    ...
    print("⚠️ CORS Middleware DISABLED - Cloudflare handles CORS at the edge")
