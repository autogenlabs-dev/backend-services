...
async def assign_glm_key():
    ...
    # Find the test user
    user = await User.find_one({"email": "codemurf0@gmail.com"})
    if not user:
        print("User not found")
        return
    
    print(f"Found user: {user.name} ({user.email})")
    print(f"Current subscription: {user.subscription}")
    print(f"Current GLM key: {user.glm_api_key}")
    print(f"Current OpenRouter key: {user.openrouter_api_key}")
    
    if user.glm_api_key:
        print("User already has GLM key assigned.")
        return
    
    # Find an available GLM key
    ...
    for key in glm_pool:
        ...
        if current_users < key.max_users:
            print(f"Assigning key: {key.label} ({current_users}/{key.max_users})")
            ...
            print(f"Key preview: {key.key_value[:10]}...")
            return
